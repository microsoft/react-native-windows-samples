...
The React Native WinRT repo includes a code sample and documentation to get started!
...
React Native WinRT project was motivated by our goal to make Windows development easier and more natural across various programming languages through language projections—other existing projects under the same umbrella include [C#/WinRT](http://aka.ms/cswinrt), [C++/WinRT](http://aka.ms/cppwinrt), and [Rust for Windows](https://docs.microsoft.com/windows/dev-environment/rust/rust-for-windows).
...
First, you will need to copy and add the WinRTTurboModule project from the appropriate release branch depending on the version of React Native for Windows your app is using (for example, release/0.67). Add this project to your React Native for Windows solution and turn off the “Deploy” setting for the project. Then you can specify the WinRT namespaces you want to project using the `RnWinRTParameters` property in WinRTTurboModule.vcxproj. In Visual Studio, you will need to unload the project before editing the file:
```xml
<RnWinRTParameters>-include Windows.UI.Notifications -include Windows.Data.Xml.Dom</RnWinRTParameters>
```

Now you can build the WinRTTurboModule project to generate the React Native module containing these types. 

Next, you will need to link your React Native Windows project to the WinRTTurboModule project to reference and access the projected WinRT namespaces. This includes the following modifications:
...
- Register the module in your React Native for Windows app in App.cpp, inside the App constructor:
  ...
  ```
- Load and initialize the module in your JavaScript code. First, create a new file called WinRTTurboModule.js, and paste the following code:
  ```javascript
