...
> **Important:** Starting with React Native Windows 0.82, the legacy Paper architecture has been completely removed. All apps now run exclusively on the [New Architecture (Fabric)](https://reactnative.dev/architecture/landing-page). If you haven't already migrated your app to Fabric, you must do so before upgrading to 0.85. See the [New Architecture migration guide](new-architecture.md) for details.
...
```bat
npx --yes @react-native-community/cli@latest init <projectName> --version 0.85.0
```
...
```bat
yarn add react-native-windows@^0.85.0
```
...
```bat
npm install --save react-native-windows@^0.85.0
```
