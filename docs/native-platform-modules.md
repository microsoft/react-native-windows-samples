...
1. Define the API surface for your Native Module in one or more TypeScript spec files
...
This spec file declares that React Native expects every platform to implement a Turbo Native Module names `Testlib` which implements a single function named `multiply` with the given arguments and return type.

> **Note**: Every spec file must be named in the format `Native<moduleName> + .ts` in order to be correctly identified as a a spec file and not just a regular TypeScript file in your library.

<!-- TODO: Add FancyMath example. -->

### 2. Use React Native for Windows' Native Module Codegen

Now, before we can implement the native C++ code for our Turbo Native Module(s), we need to run React Native for Windows' Native Module codegen, i.e. the [codegen-windows command](codegen-windows-cli.md), which will take the TypeScript spec files and generate some C++ headers with the API surface we need to implement.
...
Now we should see some files in the `windows/testlib/codegen` folder we specified in our config. Specifically, for our `Testlib` module, we should see a `NativeTestlibSpec.g.h` with:
...
Now with the codegen complete, it's finally time to implement the `Testlib` module and it's `multiply()` function. Conveniently our new project already includes a `testlib.h` and `testlib.cpp` with implementations we can look at:
...
As you can see, the `testlib.h` file defines a `Testlib` struct, annotated with `REACT_MODULE` to signify to React Native for Windows that this struct contains the implementation of the turbo module named `Testlib`.
...
> **Note:** When adding Windows support to native modules with out-of-date (or missing) spec files, removing this line will allow your native code to compile even if it doesn't match what the JavaScript side of the library expects. This is not recommended as it risks the Windows implementation of the module going out of date.

The `Initialize()` function, annotated with `REACT_INIT`, indicating it should be run when the module is first created. Looking at the implementation in `testlib.cpp` we see it saves off a [`ReactContext`](IReactContext), which your module can later use to interact back with the JavaScript side of your code (i.e. firing JavaScript events during a native operation).

Finally, we also see the `multiply()` function, annotated with `REACT_SYNC_METHOD`, indicating that the function is synchronous. Looking at the implementation in `testlib.cpp` we see it does what we'd expect, multiplying the input parameters and returning the result.

> **Note:** By default the native project (`testlib.vcxproj` in our example) already included the `testlib.h` and `testlib.cpp` files. But if you create new files for your module, you'll need to add those to the native project manually.
...
Since the purpose of the library is to expose the native functionality to code outside of the library (aka our React Native for Windows app code), the default is to export the functionality in the project's index, in this case, in `index.tsx`:
